...
| **Report** | Self-contained combined artifact after agents run: raw agent outputs (including discovered on-disk research artifacts) + cross-summary evaluated against `task.md`. |
| **Cross-summary** | Comparison of two agent outputs evaluated against `task.md` (task coverage / consensus / diffs / watch-outs), currently via Gemma 4 31B. |
