...
class TestGemmaTimeout:
    ...
    async def test_generate_cross_summary_timeout(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr("research_pipeline.clients.gemma.GOOGLE_API_KEY", "dummy_key")
        ...
        monkeypatch.setitem(pytest.importorskip("sys").modules, "google.genai", mock_genai)
        ...
        with pytest.raises(TimeoutError, match="Gemma call timed out after 1s"):
            await generate_cross_summary("task brief", "kilo data", "opencode data", timeout=1)
...
class TestBuildPrompt:
    ...
    def test_build_prompt_includes_task_content(self) -> None:
        ...
        prompt = _build_prompt("Compare sorting algorithms", "Kilo findings", "Opencode findings")
