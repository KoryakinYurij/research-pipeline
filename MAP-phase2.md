...
- **CLI isolation & parallel dispatch (owner decision 2026-07-27, [P1H-T6](tickets/P1H-T6-isolation-and-parallelism-grilling.md)):** Kilocode and Opencode run in parallel (`asyncio.gather`) with per-agent CWDs (`run-{timestamp}/kilocode/`, `.../opencode/`) and scrubbed environment (`_clean_env`). **No sandbox** — an agent can still walk up the tree and write into the repo; that residual risk is accepted for manual runs and is the **gate for unattended watch mode**.
